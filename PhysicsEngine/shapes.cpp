...
void Shape::DrawVAO(bool debug)
{
    if (debug)
    {
        ...
        glBindVertexArray(vaoID);
        CHECKERROR;
        glDrawElements(GL_LINE_STRIP, 3 * count, GL_UNSIGNED_INT, 0);
        CHECKERROR;
        glBindVertexArray(0);
    }
    else
...
void Line::MakeVAO()
{
    ...
    glGenBuffers(1, &Ibuff);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, Ibuff);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(int) * index.size(), &index[0], GL_STATIC_DRAW);
