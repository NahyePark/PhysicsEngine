...
Object::Object(const char* _name, Shape* _shape, const int _objectId, Shape* boundingShape,
    const glm::vec3 _diffuseColor, const glm::vec3 _specularColor, const float _shininess, BoundingType bt, bool rb)
    : name(_name), diffuseColor(_diffuseColor), specularColor(_specularColor), shininess(_shininess),
    shape(_shape), objectId(_objectId), drawMe(true), hasRigidBody(rb), rigidbody(boundingShape, bt)

{}


void Object::Draw(ShaderProgram* program)
{
    ...
    if (hasRigidBody)
    {
        loc = glGetUniformLocation(program->programId, "lineColor");
        glUniform3fv(loc, 1, &rigidbody.m_collider->m_color[0]);
        ...
        glUniformMatrix4fv(loc, 1, GL_FALSE, Pntr(rigidbody.m_collider->m_objTr));

        glm::mat4 inv = glm::inverse(rigidbody.m_collider->m_objTr);
    ...
    if (shape)
        if (drawMe)
        {
            ...
            if (hasRigidBody)
            {
                ...
                if (debugDraw->colliderDrawing)
                {
                    rigidbody.m_collider->Draw();
                }
                if (debugDraw->velocityDrawing)
                {
                    loc = glGetUniformLocation(program->programId, "ModelTr");
                    ...
                    linearVelocity.Pnt[0] = glm::vec4(rigidbody.m_collider->m_position,1.0);
                    linearVelocity.Pnt[1] = glm::vec4(rigidbody.m_collider->m_position+rigidbody.Velocity(), 1.0);
                    
                    linearVelocity.Update();
                    linearVelocity.DrawVAO();

                    angularVelocity.Pnt[0] = glm::vec4(rigidbody.m_collider->m_position, 1.0);
                    angularVelocity.Pnt[1] = glm::vec4(rigidbody.m_collider->m_position + rigidbody.AngularVelocity(), 1.0);

                    angularVelocity.Update();
                    angularVelocity.DrawVAO();
                }
            }
        }
    CHECKERROR;
    ...
    if (drawMe)
        for (int i=0;  i<instances.size();  i++) {
            ...
            instances[i].first->Draw(program); }
